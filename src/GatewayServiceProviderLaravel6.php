...
class GatewayServiceProviderLaravel6 extends ServiceProvider
{
	...
	public function boot()
	{
        ...
        //php artisan vendor:publish --provider=HamidNE\Gateway\GatewayServiceProvider --tag=config
        $this->publishes([
            $config => config_path('gateway.php'),
        ], 'config')
        ;

        // php artisan vendor:publish --provider=HamidNE\Gateway\GatewayServiceProvider --tag=migrations
        $this->publishes([
        ...
        // php artisan vendor:publish --provider=HamidNE\Gateway\GatewayServiceProvider --tag=views
        $this->publishes([
